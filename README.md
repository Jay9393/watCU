...
- 왓챠피디아 기능(평가 등)을 활용하여 편의점 브랜드 CU의 상품을 소개하는 기획을 추가한 클론 프로젝트입니다.
...
> Back-End : Python, Django,  PyJWT, Bcrypt, Mysql, AqueryTool, AWS(EC2,RDS)

### 협업 **도구**

- Slack / Git + GitHub / Trello 를 이용해서 버전 관리 및 작업을 에자일하게 했습니다.
...
- **ProductView (쿼리파라미터 이용한 카테고리 필터링, annotate 이용한  레이팅 높은 순으로 정렬)**
- **PrivateProductView (데코레이터 이용한 토큰검사, 쿼리파라미터 이용한 페이지네이션, 카테고리 필터링)**
- **ProductDetailView (상세 정보, 해당 상품 다른 이미지들)**

### Rating

- **RatingView (get method -> 레이팅 읽기, post method -> 레이팅 추가 및 수정, delete method -> 레이팅 삭제)**
- **RatingGraphView (해당 상품에 대한 별점 모두 읽기)**
- **RatingCountView (모든 상품에 대한 모든 별점 수 카운트)**
